...
public class TooltipUI : MonoBehaviour
{
    ...
    void RefreshTooltipUIContent(Placement placement)
    {
        ...
        if (Utilities.GetRootComponent<GameManager>().EnableDebugTools == true)
        {
            ...
            tooltipPlacementPassableText.text = (placement.IsPassable ? "Passable" : "Impassable") + (cell.HasValue && distances.ContainsKey(cell.Value) ? (distances[cell.Value].Passable ? " passable: " + distances[cell.Value].UnexploredDistance : " impassible") : -1);

        }
        else
        {
            tooltipPlacementNameText.text = placement.Name;
            tooltipPlacementPassableText.text = placement.IsPassable ? "Passable" : "Impassable";
        }
